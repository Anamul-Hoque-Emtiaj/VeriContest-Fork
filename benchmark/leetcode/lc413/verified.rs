...
impl Solution {
        ...
        decreases k,
    {
        if k < 2 {
            ...
            Self::spec_total(nums, k - 1) + Self::spec_run(nums, k)
        }
    }
    ...
    pub open spec fn spec_number_of_arithmetic_slices(nums: Seq<i32>) -> int
    {
        if nums.len() < 3 {
            ...
            Self::spec_total(nums, (nums.len() - 1) as int)
        }
    }
    ...
    proof fn lemma_run_bounds(nums: Seq<i32>, i: int)
        ...
        ensures
            0 <= Self::spec_run(nums, i),
            Self::spec_run(nums, i) <= if i < 2 { 0int } else { i - 1 },
        decreases i,
    {
        if i < 2 {
        } else if nums[i] - nums[i - 1] == nums[i - 1] - nums[i - 2] {
            Self::lemma_run_bounds(nums, i - 1);
        } else {
        }
    }
    ...
    proof fn lemma_total_bounds(nums: Seq<i32>, k: int)
        requires
            ...
            forall|j: int| 0 <= j < nums.len() ==> -1000 <= #[trigger] nums[j] <= 1000,
        ensures
            0 <= Self::spec_total(nums, k),
            Self::spec_total(nums, k) <= k * k,
        decreases k,
    {
        if k < 2 {
        } else {
            Self::lemma_total_bounds(nums, k - 1);
            Self::lemma_run_bounds(nums, k);
            assert(Self::spec_total(nums, k) <= (k - 1) * (k - 1) + (k - 1)) by (nonlinear_arith)
                requires
                    Self::spec_total(nums, k - 1) <= (k - 1) * (k - 1),
                    Self::spec_run(nums, k) <= k - 1,
                    Self::spec_total(nums, k) == Self::spec_total(nums, k - 1) + Self::spec_run(nums, k),
            {
            };
    ...
    fn number_of_arithmetic_slices(nums: Vec<i32>) -> (result: i32)
        ...
        ensures
            result as int == Self::spec_number_of_arithmetic_slices(nums@),
    {
        let n = nums.len();
        if n < 3 {
            return 0;
        ...
        while i < n
            invariant
                ...
                n <= 5000,
                forall|j: int| 0 <= j < nums.len() ==> -1000 <= #[trigger] nums@[j] <= 1000,
                curr as int == Self::spec_run(nums@, (i - 1) as int),
                total as int == Self::spec_total(nums@, (i - 1) as int),
                0 <= curr,
        ...
        {
            proof {
                Self::lemma_run_bounds(nums@, i as int);
